...
class DatabaseInterface {
  ...
  Future<void> open() async {
    ...
    firebaseApp = await Firebase.initializeApp(
      ...
      options: const FirebaseOptions(
        iosClientId: "784157732504-upears5ba27c7r3qbmq7qhc6hkdm2jjl.apps.googleusercontent.com",
        projectId: "funkmusic-4387d",
        storageBucket: "funkmusic-4387d.appspot.com",
        appId: '1:784157732504:android:4a02b034b542eb23',
  ...
  Future<List<Details>> getPlaylists() async {
    ...
    try {
      (result.snapshot.value as Map).forEach((key, val) {
  ...
  Future<List> getSongs(String playlistName) async {
    var result =
        await reference.child(toPath(playlistName)).orderByPriority().once();
    List<Details> body = [];
    try {
       (result.snapshot.value as Map).forEach((key, val) {
        var item;
        ...
        else if (val["songs"] != null) item = Album.fromJson(val);
  ...
  Future createPlaylist(String playlist) {
    Details details = Details(
        title: playlist,
        thumbnail:
            "https://a10.gaanacdn.com/images/artists/63/1248963/crop_175x175_1248963.jpg",
        artists: ["0 Songs"],
        type: "album",
        url: createUrl(playlist));
    return reference
        .child("collections")
        .child(toPath(playlist))
        .set(details.toMap());
  }

  Future<DatabaseEvent> isFavourite(Details song) {
  ...
  Future addToPlaylist(List<Details> songs, String playlistName) {
    ...
    map.putIfAbsent("title",
        () => playlistName[0].toUpperCase() + playlistName.substring(1));
    map.putIfAbsent(
        "thumbnail", () => songs.length == 0 ? "" : songs[0].thumbnail);
    map.putIfAbsent("type", () => "album");
    map.putIfAbsent("url", () => createUrl(playlistName));
    songs.forEach((song) {
      reference
          .child(toPath(playlistName))
          .child(toPath(song.title))
          .set(song.toMap());
    });
    return reference.child("collections").child(toPath(playlistName)).set(map);
  }

  Future<DatabaseEvent> isPublic(Details details) {
    return FirebaseDatabase.instance
        .reference()
        .child("public")
        .child(toPath(uniqueId + details.title))
        .once();
  }
