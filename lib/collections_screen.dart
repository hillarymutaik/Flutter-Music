    ...
    return CupertinoButton(
      onPressed: () {
        if (widget.songs != null) {
          ...
          key.currentState
              .showSnackBar(SnackBar(content: Text("Song added to Playlist")));
        } else {
          Navigator.of(context)
              .push(MaterialPageRoute(builder: (context) => SongsScreen(data)));
        }
      },
  ...
  void _showDialog() {
    showDialog(
      ...
      builder: (BuildContext context) => AlertDialog(
        ...
        title:
            Text("Create Playlist", style: Theme.of(context).textTheme.headline1),
        content: TextField(
...
class SongsScreenState extends State<SongsScreen>
  ...
  Widget getSongs() {
    return StreamBuilder(
      stream:
          databaseInterface.getSongs(widget.playlistDetails.title).asStream(),
      builder: (context, snapshot) {
        if (!snapshot.hasData) {
          return Center(
            child: CircularProgressIndicator(),
  ...
  void _showDialog() {
    showDialog(
      context: context,
      builder: (BuildContext context) => AlertDialog(
        title: Text("Do you want to delete playlist?",
            style: Theme.of(context).textTheme.headline1),
        content: Text(
            "By deleting playlist you will not be able recover it again.",
        ...
        builder: (context) {
          return CupertinoActionSheet(
            actions: <Widget>[
              ...
              CupertinoActionSheetAction(
                  onPressed: () {},
                  child: SocialButton(widget.playlistDetails)),
            ],
            cancelButton: CupertinoActionSheetAction(
              onPressed: () {
                Navigator.of(context).maybePop();
