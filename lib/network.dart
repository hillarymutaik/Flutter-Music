...
class Network {
  ...
  static Future getDetails(url) async {
    ...
    if (value["songs"] != null) {
      item = List<Song>.from(
          value["songs"].map((item) => (Song.fromJson(item))).toList());
      result["songs"] = item;
    }
    if (value["artists"] != null) {
      var list = List<Artist>.from(
          value["artists"].map((item) => (Artist.fromJson(item))).toList());
      result["artists"] = list;
    }
    if (value["albums"] != null) {
      var list = List<Details>.from(
          value["albums"].map((item) => (Details.fromJson(item))).toList());
      result["albums"] = list;
    }
    return result;
  }
