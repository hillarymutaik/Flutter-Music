...
class MyApp extends StatefulWidget {
  ...
  const MyApp(
      {Key key, this.brightness = Brightness.dark, this.startScreen = false})
      : super(key: key);
  @override
  ...
  Widget build(BuildContext context) {
    return new MaterialApp(
      ...
      theme: new ThemeData(
          ...
          textTheme: TextTheme(
            headline1: TextStyle(fontWeight: FontWeight.w600, fontSize: 18.0),
            subtitle1: TextStyle(fontSize: 16.0, fontWeight: FontWeight.w600),
...
class SoundWaveState extends State<SoundWave>
    with TickerProviderStateMixin<SoundWave> {
  //Stream Controller
  static final StreamController<Command> streamController =
      StreamController<Command>();
  static final PublishSubject<Command> publishSubject =
      PublishSubject<Command>();
    ...
    showDialog(
        ...
        builder: (context) {
          return AlertDialog(
            ...
            content: Text(
              "Sorry this feature is not available now. Would you like to visit Github Page?",
              style: Theme.of(context).textTheme.headline1,
            ),
            actions: <Widget>[
              FlatButton(
  ...
  static void changePlayingState({bool playing = true}) {
    ...
    publishSubject.sink
        .add(Command(command: Command.playingState, data: playing));
  }
  ...
  static void createPosition(Duration value) {
    publishSubject.sink
        .add(Command(command: Command.createPosition, data: value));
  }
...
//
  ...
  Widget createBottom() {
    ...
    widgets.add(BottomNavigationBar(
      ...
      fixedColor: Theme.of(context).textTheme.headline1.color,
      iconSize: 24.0,
  ...
  List<Widget> createBottomItems() {
    ...
    return items
        .map(
            (item) => ImageIcon(AssetImage("images/${item.toLowerCase()}.png")))
        .toList();
  }
  ...
  List<BottomNavigationBarItem> createBottomNavigation() {
    ...
    return items.map((item) {
      ...
      return BottomNavigationBarItem(
          icon: ImageIcon(
            AssetImage("images/${item.toLowerCase()}.png"),
            size: size,
          ),
          label: item,
          // headline: Padding(
...
class Playlist {
  ...
  int _active = -1;
  bool _playing = false;
  set active(int index) =>
      _active = (index >= 0 && index < songs.length ? index : 0);
  int get active => _active;
  Song get activeSong => songs.length != 0
      ? songs[_active]
      : Song(
          links: [],
          thumbnail: "",
